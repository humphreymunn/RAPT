...
2. Isolation Forest (Statistical, catches outliers)
"""
...
parser = argparse.ArgumentParser(description="Test OOD detection with Hybrid (IsoForest + Range) detector.")
...
parser.add_argument("--train_data_path", type=str, required=True, 
                    help="Path to HDF5 training data (required for Range Detector and IsoForest training)")
parser.add_argument("--padding_epsilon", type=float, default=1e-4, 
...
parser.add_argument("--iso_contamination", type=float, default=0.01,
                    help="Expected contamination ratio for Isolation Forest (default: 0.01)")
parser.add_argument("--iso_n_estimators", type=int, default=100,
...
parser.add_argument("--output_dir", type=str, default="ood_results_isoforest",
...
class OODInjector:
    ...
    def _setup_noise(self):
        """Setup noise: add Gaussian noise with std 0.05 to 0.15 on 1-5 obs."""
        num_affected = torch.randint(1, 6, (self.num_ood_envs,), device=self.device)
...
# ==================================================================================
# HYBRID DETECTOR (Range + Isolation Forest)
# ==================================================================================
...
class IsolationForestDetector:
    def __init__(self, train_data_path: str, n_estimators: int = 100, contamination: float = 0.01):
        print(f"[INFO] Initializing Isolation Forest from: {train_data_path}")
        ...
        if os.path.exists(train_data_path):
            with h5py.File(train_data_path, 'r') as hf:
                observations_ds = hf['observations']
                if not isinstance(observations_ds, h5py.Dataset):
                    raise ValueError("Expected 'observations' to be an HDF5 dataset")
                data = np.asarray(observations_ds[:])

            data = self._to_feature_matrix(data, source="training")
                
            print(f"[INFO] Training Isolation Forest on {len(data)} samples...")
            start_t = time.time()
            self.model.fit(data)
            print(f"[INFO] Training done in {time.time() - start_t:.2f}s")
        else:
            raise FileNotFoundError(f"Training data not found for Isolation Forest: {train_data_path}")

    @staticmethod
    def _to_feature_matrix(data: np.ndarray, source: str = "input") -> np.ndarray:
        arr = np.asarray(data)

        if arr.ndim == 0:
            raise ValueError(f"IsolationForest {source} data must have at least 1 dimension, got scalar")
        if arr.ndim == 1:
    ...
    def score_samples(self, obs_numpy: np.ndarray) -> np.ndarray:
        """
        Returns anomaly scores.
        Lower scores = More anomalous (standard IF definition).
        We negate them so Higher = More Anomalous.
        """
        obs_numpy = self._to_feature_matrix(obs_numpy, source="inference")

        # decision_function returns negative values for outliers, positive for inliers
        # We negate it so: High Positive = Anomaly, Negative = Normal
        scores = -self.model.decision_function(obs_numpy)
        return scores

class HybridAnomalyDetector:
    """
    Combines Simple Range Detection + Isolation Forest.
    Anomaly = Range_Anomaly OR IsoForest_Anomaly.
    """
    def __init__(self, train_data_path: str, obs_dim: int, 
                 n_estimators: int = 100, padding_epsilon: float = 1e-4, margin_percent: float = 0.05, device: str = 'cuda'):
        
        self.device = device
        self.obs_dim = obs_dim
        self.if_threshold = 0.0 # Default (decision boundary), will be calibrated
        ...
        self.iso_forest = IsolationForestDetector(train_data_path, n_estimators=n_estimators)
            
        print(f"[INFO] Hybrid (Range + IsoForest) Detector Initialized.")

    def calibrate(self, obs: torch.Tensor):
        """
        Updates Range bounds and returns IF scores for threshold calibration.
        """
        if obs.device != self.device: obs = obs.to(self.device)
        ...
        scores = self.iso_forest.score_samples(obs_cpu)
    ...
    def check_batch(self, obs: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        # 2. IsoForest Check (CPU)
        obs_cpu = obs.cpu().numpy()
        scores_np = self.iso_forest.score_samples(obs_cpu)
        scores_tensor = torch.from_numpy(scores_np).to(self.device)
        
        is_if = scores_tensor > self.if_threshold
        
        # 3. Combine
        is_anomaly = is_range | is_if
        
        return is_anomaly, is_range, is_if, scores_tensor
@dataclass
class EnvResult:
    env_idx: int
    is_ood_env: bool 
...
class EpisodeResult:
    ...
    tp_range_only: int = 0
    tp_forest_only: int = 0
    tp_both: int = 0
    tpr_range: float = 0.0
    fpr_range: float = 0.0
    tpr_forest: float = 0.0
    fpr_forest: float = 0.0
    
    # ROC-related data (store raw scores for later AUROC calculation)
    ...
    def compute_metrics(self):
        ...
        self.tp_forest_only = 0
        self.tp_both = 0
        
        tp_range = fp_range = tn_range = fn_range = 0
        tp_forest = fp_forest = tn_forest = fn_forest = 0
        tp_steps = []
        padd_accum = 0.0
        ood_total_count = 0
        ...
        for r in self.env_results:
            ...
            if is_ood:
                ...
                if r.detected:
                    ...
                    if r.detected_by_range and r.detected_by_forest:
                        self.tp_both += 1
                    elif r.detected_by_range:
                        self.tp_range_only += 1
                    elif r.detected_by_forest:
                        self.tp_forest_only += 1
                else:
            ...
            if is_ood:
                if r.detected_by_forest: tp_forest += 1
                else: fn_forest += 1
            else:
                if r.detected_by_forest: fp_forest += 1
                else: tn_forest += 1
        ...
        self.tpr_forest = tp_forest / total_ood if total_ood > 0 else 0.0
        self.fpr_forest = fp_forest / total_control if total_control > 0 else 0.0
...
class ResultsTracker:
    ...
    def record_detection(self, episode: EpisodeResult, env_idx: int, step: int, 
        ...
        if not result.detected:
            ...
            if by_svdd: result.detected_by_forest = True
    ...
    def finalize_episode(self, episode: EpisodeResult):
        ...
        print(f"{'-'*60}")
        print(f"  Overlap (TPs): Range Only: {episode.tp_range_only}, forest Only: {episode.tp_forest_only}, Both: {episode.tp_both}")
        print(f"{'='*60}\n")
        ...
        with open(output_path, 'w', newline='') as f:
            ...
            for ep in self.episodes:
                writer.writerow([
                    ...
                    ep.tp_range_only, ep.tp_forest_only, ep.tp_both
                ])
...
def main():
    ...
    hybrid_detector = HybridAnomalyDetector(
        ...
        obs_dim=obs_dim,
        n_estimators=args_cli.iso_n_estimators,
    ...
    try:
        ...
        for step in range(args_cli.episode_length):
            ...
            is_anomaly, is_rng, is_if, _ = hybrid_detector.check_batch(obs)
        ...
        # 2. IsoForest Threshold:
        all_s = torch.stack(baseline_scores).max(dim=0)[0] # Max score per env
        new_if_threshold = float(np.percentile(all_s.cpu().numpy(), 99.73))
        hybrid_detector.if_threshold = new_if_threshold

        iso_model_path = os.path.join(results_tracker.output_dir, "isolation_forest.joblib")
        joblib.dump(hybrid_detector.iso_forest.model, iso_model_path)
        
        print(f"\n[CALIBRATION COMPLETE]")
        print(f"  Range Bounds Updated.")
        print(f"  Isolation Forest Threshold Set: {new_if_threshold:.4f} (99.73%)")
    ...
    try:
        for i, category in enumerate(categories_to_test):
            ...
            for step in range(args_cli.episode_length):
                ...
                # HYBRID CHECK
                t_start = time.perf_counter()
                is_anomaly, is_rng, is_if, scores = hybrid_detector.check_batch(obs_perturbed_current)
                t_end = time.perf_counter()
                ...
                for env_idx in range(num_envs):
                    if is_anom_cpu[env_idx] and not detected_envs[env_idx]:
                        results_tracker.record_detection(episode_result, env_idx, step, s_cpu[env_idx], 
                                                       is_rng_cpu[env_idx], is_if_cpu[env_idx])
                        detected_envs[env_idx] = True
                    else:
                        results_tracker.update_score(episode_result, env_idx, s_cpu[env_idx])
